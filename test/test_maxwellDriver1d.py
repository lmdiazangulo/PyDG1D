...
def test_periodic_LSERK_errors():
    ...
    t = 0.0   
    for _ in range(100):
        plt.plot(sp.x, sinusoidal_wave_function(sp.x, drLSERK54.timeIntegrator.time), 'g')
...
def test_periodic_implicit_errors():
    ...
    for _ in range(100):
        plt.plot(sp.x, sinusoidal_wave_function(sp.x, drCN.timeIntegrator.time), 'g')
...
def test_errors():
    ...
    error_RMSE_LSERK54 = np.sqrt(np.sum(real_function(sp.x, drLSERK54.timeIntegrator.time)-(drLSERK54['E']))**2)/drLSERK54['E'].size
    error_RMSE_LSERK134 = np.sqrt(np.sum(real_function(sp.x, drLSERK134.timeIntegrator.time)-(drLSERK134['E']))**2)/drLSERK134['E'].size
    error_RMSE_CN = np.sqrt(np.sum(real_function(sp.x, drCN.timeIntegrator.time)-(drCN['E']))**2)/drCN['E'].size
    error_RMSE_IBE = np.sqrt(np.sum(real_function(sp.x, drIBE.timeIntegrator.time)-(drIBE['E']))**2)/drIBE['E'].size
